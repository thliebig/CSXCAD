...
from vtkmodules.vtkFiltersGeneral import vtkTransformPolyDataFilter, vtkBooleanOperationPolyDataFilter
...
def rotationalExtrude(src_port, rot_axis, angle_range, arc_resolution=8):
    ...
    InvTransform = vtkTransform()
    transformFilter = vtkTransformPolyDataFilter()
    InvTransformFilter = vtkTransformPolyDataFilter()
...
def createCylinderPoly(start, stop, radius, arc_resolution=8):
    ...
    transform = vtkTransform()
    tr_flt = vtkTransformPolyDataFilter()
...
def createCylinderShellPoly(start, stop, radius, width, arc_resolution=8):
	...
	# extrude in +z
    linearExtrusionFilter = vtkLinearExtrusionFilter()
...
def transformCylindricalCoords(coord, src_CS=CSRectGrid.CYLINDRICAL):
    if src_CS is not CSRectGrid.CYLINDRICAL:
        return coord
...
def convertPrim2Polydata(prim, default_CS=CSRectGrid.CARTESIAN, arc_resolution=16):
    pt = prim.GetType()
    ...
    if pt == CSPrimitives.POINT:
        pass
    elif pt == CSPrimitives.BOX and prim_cs in [None, CSRectGrid.CARTESIAN]:
    ...
    elif pt == CSPrimitives.BOX and prim_cs == CSRectGrid.CYLINDRICAL:
        start, stop = sortStartStop(prim.GetStart(), prim.GetStop())
        poly_data = createCylindricalCubePoly(start, stop)
    elif pt == CSPrimitives.SPHERE:
        src = vtkSphereSource()
        ...
        src.Update()
        poly_data = src.GetOutput()
    elif pt == CSPrimitives.SPHERICALSHELL:
        # This is not really a shell
        # somehow the vtkBooleanOperationPolyDataFilter does not work anymore as it used too!
        src_o = vtkSphereSource()
        r = prim.GetRadius()
    ...
    elif pt == CSPrimitives.CYLINDRICALSHELL:
        start, stop = sortStartStop(prim.GetStart(), prim.GetStop())
        start = transformCylindricalCoords(start, prim_cs)
        stop  = transformCylindricalCoords(stop, prim_cs)
        poly_data = createCylinderShellPoly(start, stop, prim.GetRadius(), prim.GetShellWidth(), arc_resolution=arc_resolution)
    elif pt in [CSPrimitives.POLYGON, CSPrimitives.LINPOLY, CSPrimitives.ROTPOLY]:
        if prim_cs == CSRectGrid.CYLINDRICAL:
    ...
    elif pt in [CSPrimitives.POLYHEDRON, CSPrimitives.POLYHEDRONREADER]:
        if prim_cs == CSRectGrid.CYLINDRICAL:
    ...
    elif pt in [CSPrimitives.CURVE, CSPrimitives.WIRE]:
        N = prim.GetNumberOfPoints()
        if N<2:
            return None
        coords = [transformCylindricalCoords(prim.GetPoint(n), prim_cs) for n in range(N)]
        radius = None if pt == CSPrimitives.CURVE else prim.GetWireRadius()
...
def renderPrimitives(ren, prop, default_CS=CSRectGrid.CARTESIAN, arc_resolution=8):
    apd = vtkAppendPolyData()
    has_any = False
