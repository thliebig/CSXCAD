...
for n=1:numel(diff_lines)-1
    if (diff_lines(n+1)/diff_lines(n) > ratio*1.01)
        warning('CSXCAD:CheckMesh',['found resolution increase larger than ratio: ' num2str(diff_lines(n+1)/diff_lines(n)) ' > ' num2str(ratio) ]);
        pos = [pos n];
        EC = EC + 1;
    end
    if (diff_lines(n+1)/diff_lines(n) < 1/ratio/1.01)
        warning('CSXCAD:SmoothRange',['found resolution decrease smaller than ratio: ' num2str(diff_lines(n+1)/diff_lines(n)) ' < 1/' num2str(ratio) '=' num2str(1/ratio) ]);
        pos = [pos n];
        EC = EC + 1;
    end
end